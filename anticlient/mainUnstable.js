...
var loadProjectileModules = () => {
  ...
  const TrajectoryInfo = {
    BOW_FULL: { gravity: 0.05, hitboxRadius: 0.5, initialVelocity: 3, drag: 0.99 },
    ...
    FISHING_ROD: { gravity: 0.04, hitboxRadius: 0.25, initialVelocity: 1, drag: 0.92 }
  };
  class Vec3 {
    constructor(x, y, z) {
      this.x = x || 0;
  ...
  class ProjectileCalculator {
    ...
    static getDirectionByTime(trajectoryInfo, enemyPos, playerHeadPos, time) {
      ...
      return new Vec3(
        ...
        diff.y * fResistance / denominator + g * (r_t - r * time + time - 1) / (vA * fResistance * (r_t - 1)),
     ...
     */
    static findMinimumByBisect(min, max, fn, maxIterations = 25, tolerance = 1e-3) {
      let left = min;
      let right = max;
      let iterations = 0;
    ...
    static calculateTravelTime(trajectoryInfo, targetPos, playerHeadPos, maxTime = 100) {
      ...
      const { ticks, delta } = this.findMinimumByBisect(0, maxTravelTime, (t) => {
        ...
        return Math.abs(direction.length() - 1);
      });
  ...
  bowAimbot.onTick = (bot) => {
    ...
    if (!trajectoryInfo) {
      logger2.warn(`Unknown projectile type: ${settings.projectileType}`);
      return;
    }
    const filter = settings.targetType === "players" ? ((e) => e.type === "player") : settings.targetType === "mobs" ? ((e) => e.type === "mob") : ((e) => e.type === "player" || e.type === "mob");
    currentTarget = bot.nearestEntity(
      (e) => filter(e) && e !== bot.entity && e.position.distanceTo(bot.entity.position) < settings.range
    );
    if (!currentTarget) {
  ...
  bowAimbot.onToggle = (enabled) => {
    ...
    logger2.info(`Bow Aimbot ${enabled ? "enabled" : "disabled"}`);
  };
  registerModule(bowAimbot);
  ...
  projectileESP.onToggle = (enabled) => {
    ...
    logger2.info(`Projectile ESP ${enabled ? "enabled" : "disabled"}`);
  };
  registerModule(projectileESP);
  logger2.info("Projectile modules loaded (Bow Aimbot, Projectile ESP)");
};
